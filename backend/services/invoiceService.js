...
function fmt(n) {
  return Number(n).toLocaleString('en-EG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' EGP';
}
...
async function buildInvoicePdf({ userId, salaryMonth }) {
  ...
  const sessions = await db.all(
    `SELECT * FROM sessions WHERE user_id = ? AND salary_month = ? ORDER BY date ASC, id ASC`,
    [userId, salaryMonth],
  );

  // Monthly breakdown for this cycle's summary
  ...
  // Payments up to cycle end
  const payments = await db.all(
    `SELECT * FROM payments WHERE user_id = ? AND date <= ? ORDER BY date ASC`,
    [userId, end],
  );
  ...
  const summaryItems = [
    ...
    ['Earned this cycle', fmt(cycleData.expectedEarnings)],
    ['Cumulative paid (to cycle end)', fmt(cycleData.cumulativePaid)],
  ];
  ...
  doc.fontSize(9).font('Helvetica-Bold').fillColor(balColor).text(fmt(Math.abs(balance)), 230, y);
  y += 20;
  ...
  sessions.forEach((s, i) => {
    ...
    tableRow(
      doc,
      [s.date, s.title, s.category, s.day_of_week || '', s.duration_hours.toFixed(2), fmt(s.rate_applied), fmt(s.earnings)],
      y, SW,
      { bg: i % 2 === 1 ? '#f8fafc' : null, fontSize: 8 },
    );
    y += 15;
  });
  ...
  payments.forEach((p, i) => {
    if (y > doc.page.height - 80) { doc.addPage(); y = 50; }
    tableRow(doc, [p.date, fmt(p.amount_egp), p.note || ''], y, PW, { bg: i % 2 === 1 ? '#f8fafc' : null, fontSize: 8 });
    y += 15;
  });
